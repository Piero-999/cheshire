...
# The ILA exists only in the debug build, so that is what these scripts program
# and whose probes they load; the flow (config.py) uses the build without it.
# An explicit BIT or LTX still wins.
export BIT="${_bit:-$REPO/target/xilinx/out/with_ila/cheshire.zcu102.bit}"
unset _bit
...
export LTX="${LTX:-$REPO/target/xilinx/out/with_ila/cheshire.zcu102.ltx}"
